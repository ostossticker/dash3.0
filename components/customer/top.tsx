...
const Customertop = ({val,setBus, func,val1,func1,onclick,suggesting}:customerTopProps) => {
                                            ...
                                            <ul className='absolute rounded-md border-[1px] shadow-md bg-white pl-2  py-1 w-full mt-2'>
                                                {
                                                    suggesting.map((op)=>{
                                                        return(
                                                            <li key={op.id} className='cusor-pointer' onClick={()=>{
                                                            handleBlur(index)
                                                            item.typeSelect === "cusBus" && handleClick(op)}}>
                                                                {item.typeSelect === "cusBus" && op.cusBus  }
                                                            </li>
                                                        )
                                                    })
                                                }
