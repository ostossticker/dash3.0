"use client"
import { addMeter } from "@/app/(protected)/invoice/actions/meters";
...
import Newtest1 from "./invoice_form";
...
const Meters = ({busType,cusName,toggleEmail , toggleAddr , togglePo ,invStatus ,abaName , abaNumber , partial , discount , cusComp , cusPhone ,cusEmail,cusAddr,invBus,invNo ,invPo ,invTitle,invDate}:meterProps) => {
...
const calculateBalance = () => {
  ...
  const save = () =>{
    ...
    if(!cusName || !cusPhone){
      ...
      addMeter({
        ...
        if(data?.saving){
          alert(JSON.stringify(data.saving))
          setPending(false)
        }
      }).catch(()=>{
  ...
  return (
    <>
    <div className={`${busType === "meter" ? "block" : "hidden"} `}>
      ...
      <table className="w-full">
        ...
        >
          <td className="py-2"
          >
            <div className={`cursor-grabbing flex justify-center items-center border py-1 mx-[10px] border-insomnia-primary w-[34px] rounded-md ${darkMode ? "text-dark-lg-color" : ""}`}
          ...
          <td className=" px-[10px] py-2">
            <input
              type="text"
              className={`${darkMode ? "text-dark-lg-color" : ""} xl:w-[600px] 2xl:w-[1100px] border px-2 py-1 bg-transparent border-insomnia-primary rounded-md `}
              value={item.description}
              onChange={(e) => handleChange(index, 'description', e.target.value)}
            />
          </td>
          <td className="px-[10px] py-2">
            <div className="flex justify-center items-center border border-insomnia-primary rounded-md">
            <input
              type="number"
              value={item.sizeWidth === 0 ? '' : item.sizeWidth}
              onChange={(e) => handleChange(index, 'sizeWidth', e.target.value)}
              className={`${darkMode ? "text-dark-lg-color" : ""}  px-2 py-1 xl:w-[70px] bg-transparent text-end outline-none 2xl:w-[80px]`}
            />
            <div className={`${darkMode ? "text-dark-lg-color" : ""} mx-[20px] px-3 flex justify-center items-center py-1 bg-transparent `}>
              x
            </div>
            <input
              type="number"
              ...
              className={`${darkMode ? "text-dark-lg-color" : ""} px-2 py-1 xl:w-[70px]  bg-transparent text-start outline-none 2xl:w-[80px]`}
            />
            </div>
          </td>
          <td className=" px-[10px] py-2">
            <input
              type="number"
              value={item.m2 === 0 ? '' : item.m2.toFixed(2)}
              disabled
              className={`${darkMode ? "text-dark-lg-color" : ""} w-full text-center border px-2 py-1 bg-transparent border-insomnia-primary rounded-md`}
            />
          </td>
          <td className=" px-[10px] py-2">
            <input
              ...
              className={`${darkMode ? "text-dark-lg-color" : ""} w-full border text-center px-2 py-1 bg-transparent border-insomnia-primary rounded-md`}
            />
          ...
          <div className={`flex justify-center items-center gap-2`}>
            <button className={`${darkMode ? "text-blue-400" : "text-blue-700"} border border-insomnia-primary p-1 rounded-md`} onClick={()=>handleCopy(item.description , item.sizeHeight , item.sizeHeight , item.m2 , item.quantity, item.unitPrice,item.total)}><PiCopySimpleLight size={25}/></button>
      ...
      <button onClick={handleAddCalculation} className={`${darkMode ? "text-dark-lg-color" : ""} border border-insomnia-primary rounded-md ml-[10px] px-[20px] py-[5px] text-[18px] font-bold`}>Add</button>
